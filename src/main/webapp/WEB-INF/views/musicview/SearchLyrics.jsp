...
<body>
	...
	<form id="SearchLyrics" name="SearchLyrics" align="center">
		<!-- 곡검색 -->
		<h1>곡 검색</h1>
		...
		<table style="width: 100%;" border="1">
			<tr align="center" height="2" bgcolor="ghostwhite">
				<td width="50"><input type="checkbox" id="check"
					name="check"></td>
				<td width="40">번 호</td>
				<td>Image</td>
				<td>곡 명</td>
			...
			<c:forEach var="row" items="${Durian}" varStatus="vs">
				<tr>
					<td align="center"><input type="checkbox"
						class="normalCheck_2" id="snum${row.snum}" name="snum"
						value="${row.snum}"></td>
	...
	<c:if test="${pageMaker3.prev && pageMaker3.sPageNo>1 }">
		<a href="mSearch${pageMaker3.searchQuery(1)}">First</a>&nbsp;  
		<!-- "bpage?currPage=1" -->
		<a href="mSearch${pageMaker3.searchQuery(pageMaker3.sPageNo-1)}">Prev</a>&nbsp;&nbsp;
	</c:if>
	
	<!-- 2) sPage~ePage 까지 displayPageNo 값 만큼 출력, -->
	<c:forEach var="i" begin="${pageMaker3.sPageNo}" end="${pageMaker3.ePageNo}">
		<c:if test="${i==pageMaker3.cri.currPage}">
			<font size="5" color="Orange">${i}&nbsp;</font>
		</c:if>
		<c:if test="${i!=pageMaker3.cri.currPage}">
			<a href="mSearch${pageMaker3.searchQuery(i)}">${i}</a>&nbsp;
		</c:if>
	</c:forEach>
		
	<!-- 3) Next >  ,   Last >> : enabled 여부	 -->
	<c:if test="${pageMaker3.next && pageMaker3.ePageNo>0}">
		<a href="mSearch${pageMaker3.searchQuery(pageMaker3.ePageNo+1)}">&nbsp;&nbsp;Next</a>&nbsp;  
		<a href="mSearch${pageMaker3.searchQuery(pageMaker3.lastPageNo)}">Last</a>&nbsp;&nbsp;
	</c:if>
	</div>
	<footer>
		<div id="footerinfo">
