...
$(function(){
   $('#searchBtn').on("click",function(){
      self.location="searchSingerName"
                +"?currPage=1" 
...
<body>
	...
	<nav>
		<ul>
			...
			<li><a href="javascript:;" onClick="location.reload()" style="color: #0b3f9a; font-size: 25px;">아티스트 검색</a>
	...
	<div id="searchdiv">
		<div id="logo">
			<h1>GMUSIC</h1>
			<form action="searchSingerName" id="search" name="search" class="search">
	...
	<div align="center" style="font-size: 20px;">
		...
		<c:if test="${pageMaker.prev && pageMaker.sPageNo>1}">
			<a href="searchSingerName${pageMaker.searchQuery(1)}">First</a>&nbsp;  
		<!-- "bpage?currPage=1" -->
			<a href="searchSingerName${pageMaker.searchQuery(pageMaker.sPageNo-1)}">Prev</a>&nbsp;&nbsp;
	</c:if>
		...
		<c:forEach var="i" begin="${pageMaker.sPageNo}" end="${pageMaker.ePageNo}">
			...
			<c:if test="${i!=pageMaker.cri.currPage}">
				<a href="searchSingerName${pageMaker.searchQuery(i)}">${i}</a>&nbsp;
		</c:if>
		</c:forEach>

		<!-- 3) Next >  ,   Last >> : enabled 여부	 -->
		<c:if test="${pageMaker.next && pageMaker.ePageNo>0}">
			<a href="searchSingerName${pageMaker.searchQuery(pageMaker.ePageNo+1)}">&nbsp;&nbsp;Next</a>&nbsp;  
		<a href="searchSingerName${pageMaker.searchQuery(pageMaker.lastPageNo)}">Last</a>&nbsp;&nbsp;
	</c:if>
	</div>
	<br>
	<footer>
