...
<script>
   ...
   var selectbox = {
      ...
      moveUp : function(obj) { // playlist
         ...
         var tmp ='';
         if (index != 0) {
            tmp = snumValsplit[index];
            snumValsplit[index] = snumValsplit[index-1];
            snumValsplit[index-1] = tmp;
         }
         for (var i = 0; i < snumValsplit.length - 1; i++) {
            if (snumValsplit[i] != '') {
               snumVal += snumValsplit[i] + ',';
      ...
      moveDown : function(obj) { // 마지막곡의 다음곡은 없다. 마지막곡의 이전곡부터 첫번쨰 곡까지.
         ...
         var snumVal = '';
         var tmp ='';
         var lastindex = $("#playlist option:last").prop("selected");
         if (!lastindex) {
            tmp = snumValsplit[index];
            snumValsplit[index] = snumValsplit[index+1];
            snumValsplit[index+1] = tmp;
...
<script>
   ...
   window.onkeydown = function() {
      ...
      if (kcode == 116) {
         ...
         //location.href = 'playlist?snumVal=' + $('#snumValss').val();
      }
   }
</script>
